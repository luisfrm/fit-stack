...
|---|---|
| `packages/shared` | `src/documents/` (nuevo: gate, fiscal-profile, tax-math, receipt-number, receipt-data, receipt-events, receipt-storage-keys, receipt-compose, receipt-gaps), `src/index.ts`, `src/types.ts`, `tests/documents/` |
| `packages/database` | `src/schema.ts` (+ migraciones: Panel Fase 1 = `0012`; Console C1 pendiente) |
| `apps/api-worker` | `lib/schemas.ts`, `lib/r2.ts` (+put/get), `services/receipts.service.ts` (paso 1 + `getReceiptState` + `markReceiptVoided`, sin email/render), `repositories/payments.repository.ts`, `services/subscriptions.service.ts`, `services/organizations.service.ts`, `services/reports.service.ts`, `routes/payments.route.ts`, `routes/subscriptions.route.ts` (schema), `routes/organizations.route.ts` (PATCH profile), `routes/reports.route.ts`, `routes/platform-organizations.route.ts` (schema), `index.ts` (producer `RECEIPT_QUEUE`; sin `queue()`) |
