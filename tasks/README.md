...
|---|---|
| `packages/shared` | `src/documents/` (nuevo: gate, fiscal-profile, tax-math, receipt-number, receipt-data, receipt-events, receipt-storage-keys, receipt-compose, receipt-gaps), `src/index.ts`, `src/types.ts`, `src/settings.ts` (keys emisor FitStack, C1), `tests/documents/` |
| `packages/database` | `src/schema.ts` (+ migraciones: Panel Fase 1 = `0012` (`0013` ajena al track); Console C1 = `0014` `platform_document_sequence` + columnas en `platform_subscription_payment`) |
| `apps/api-worker` | `lib/schemas.ts`, `lib/r2.ts` (+put/get), `services/receipts.service.ts` (paso 1 + `getReceiptState` + `markReceiptVoided`, sin email/render), `repositories/payments.repository.ts`, `repositories/platform-receipts.repository.ts` (nuevo, C1: secuencia global + attach idempotente), `services/subscriptions.service.ts`, `services/organizations.service.ts`, `services/reports.service.ts`, `routes/payments.route.ts`, `routes/subscriptions.route.ts` (schema), `routes/organizations.route.ts` (PATCH profile), `routes/reports.route.ts`, `routes/platform-organizations.route.ts` (schema), `index.ts` (producer `RECEIPT_QUEUE`; sin `queue()`), `tests/integration/platform-receipts-sequence.test.ts` (nuevo, C1), `tests/helpers/db.ts` (truncate `platform_document_sequence`) |
| `apps/jobs-worker` | `index.ts` (`queue()` ramificado por `batch.queue` + `scheduled()` del barrido + `Env`), `handlers/receipt.handler.ts` (nuevo: `handleReceiptRender` + `sweepPendingReceiptPdfs`), `receipt-pdf.tsx` (nuevo: render lazy `@react-pdf/renderer`, ya dependencia), `handlers/pdf.handler.ts` (adjunto, Fase 3), `wrangler.jsonc` (producers; consumer/cron viven en Terraform) |
| `packages/database` | `src/repositories/receipts.repository.ts` (excepción compartida: numeración atómica + `getReceiptComposedData` + `completeReceiptPdf` + `markReceiptNotified`) |
| `infrastructure/terraform` | `queues.tf` (cola + DLQ `fit-receipt-events{,-dlq}{env}`), `workers.tf` (producer api-worker; consumer jobs-worker + producer + cron), `main.tf` (locals de nombres), `variables.tf`/`outputs.tf`, `.github/workflows/terraform.yml` (vars `RECEIPT_QUEUE_NAME`/`RECEIPT_DLQ_QUEUE_NAME`) |
...
| `apps/console` | `app/(protected)/settings/` (+`emitter/page.tsx` UI Emisor FitStack, C1), `components/settings/emitter/emitter-settings.tsx` (nuevo, C1), `components/dashboard/settings-nav.tsx` (tab Emisor), `lib/config/platform-settings.ts` (keys `FITSTACK_*`), `e2e/console/settings.spec.ts` (2 tests Emisor: navegación + save/re-render/restore, C1), detalle suscripción/org (descarga/reenvío), `lib/services/platform-subscriptions-service.ts` |
