...
| Archivo | Cambio |
|---|---|
| `apps/api-worker/src/services/platform-receipts.service.ts` (nuevo, factory) | Mismo patrón en dos pasos de Fase 2, sobre la **misma cola `fit-receipt-events`** y el **mismo tipo de evento `receipt.render`** (DECISIÓN CERRADA: se usa un campo discriminador `scope: 'panel' \| 'platform'` dentro del payload, no un evento nuevo — el consumer `handleReceiptRender` de jobs-worker ramifica por `scope`; evita duplicar el registro de tipos de evento y mantiene un único punto de entrada de la cola). **Paso 1 síncrono** donde el pago queda validado (número global `FS-N` + impuestos + `UPDATE … WHERE receipt_number IS NULL`, responde `{ receiptNumber, pdfStatus: "pending" }`, encola render) + **paso 2 en el consumer** (compone datos: receptor org, detalle snapshots, periodo `startDate+duración`, montos incl. `baseAmount`, método enmascarado, `paymentDate` vs emisión, pie proxy → `checklistPrePdf` → render → PUT R2 `platform/receipts/<año>/FS-<n>.pdf` → `UPDATE … WHERE receipt_pdf_key IS NULL RETURNING`; **solo si `rowCount === 1`** encola `email.org_payment_received`). Sin rollback de números: `receipt_pdf_key IS NULL` = pendiente reintentable. |
...
| `packages/shared/src/documents/receipt-storage-keys.ts` (Fase 2) | `platformReceiptKey(year, receiptNumber)` (ya creado en shared). |
