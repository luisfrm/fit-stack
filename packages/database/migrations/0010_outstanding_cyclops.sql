...
CREATE TYPE "public"."org_role" AS ENUM('owner', 'manager', 'cashier', 'coach', 'member');--> statement-breakpoint
...
CREATE TABLE "fitstack_plan" (
	...
	"duration_unit" "membership_duration_unit" DEFAULT 'month' NOT NULL,
	"features" jsonb,
	"is_active" boolean DEFAULT true NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE "platform_payment" (
	"id" bigint PRIMARY KEY GENERATED ALWAYS AS IDENTITY (sequence name "platform_payment_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1),
	"organization_id" text NOT NULL,
...
ALTER TABLE "platform_payment" ADD CONSTRAINT "platform_payment_organization_id_organization_id_fk" FOREIGN KEY ("organization_id") REFERENCES "public"."organization"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "platform_payment" ADD CONSTRAINT "platform_payment_plan_id_fitstack_plan_id_fk" FOREIGN KEY ("plan_id") REFERENCES "public"."fitstack_plan"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
