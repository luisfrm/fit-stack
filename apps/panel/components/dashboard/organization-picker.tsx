...
function LoadingState({ isModal }: { isModal?: boolean }) {
  return (
  ...
  return (
    <div className="flex h-svh w-full items-center justify-center bg-background p-4">
      <div className="text-center max-w-sm">
        ...
        <Button variant="ghost-muted" onClick={onLogout} leftIcon={<LogOut size={16} />}>
          Cerrar sesión
        </Button>
      </div>
    </div>
    ...
    <div className={cn("w-full max-w-md animate-in fade-in zoom-in duration-300", !isModal && "px-4")}>
      ...
      <div className="flex flex-col divide-y divide-white/5">
        {orgs.map((org) => (
          <OrgItemRow
            ...
            isDisabled={activatingId !== null && activatingId !== org.id}
      ...
      {!isModal && (
        <div className="pt-6 border-t border-white/5 flex justify-center">
          <Button variant="ghost-muted" size="sm" onClick={onLogout} leftIcon={<LogOut size={14} />}>
            Cerrar sesión
          </Button>
        </div>
      )}
...
export function OrganizationPicker({ onSelect, isModal }: OrganizationPickerProps) {
  ...
  const [organizations, setOrganizations] = React.useState<OrgItem[]>([]);
  ...
  if (isLoading) {
    const loading = <LoadingState isModal={isModal} />;
    if (isModal) return loading;
    return (
  ...
  if (organizations.length === 0) {
    return <EmptyState onLogout={handleLogout} />;
  }
