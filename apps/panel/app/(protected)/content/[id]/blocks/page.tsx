...
import { BlockDataForm } from "@/components/content/block-forms";
import { IContentPage, IContentBlock, ContentBlockType } from "@/types/content";
import { cn } from "@workspace/ui/lib/utils";

// Tipos de bloques permitidos (vienen de los schemas compartidos en @workspace/shared)
...
export default function CMSBlocksEditor() {
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <Text as="p" weight="bold" size="lg" className="flex items-center gap-2">
        <LayoutList className="w-5 h-5 text-primary" /> Bloques de Contenido
      </Text>
    ...
    <Draggable draggableId={String(block.id)} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={cn(
            "group transition-all duration-200 animate-in fade-in slide-in-from-bottom-2",
        ...
        >
          <Card className="bg-white/5 border-none overflow-hidden relative">
            <div className="flex">
              {/* DND Handle — mismo fondo que la card para que no se vea un bloque negro */}
              <div
                {...provided.dragHandleProps}
                className="w-10 bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors cursor-grab active:cursor-grabbing"
              >
...
function BlockCreationModal({
  ...
  const handleCreate = async (type: ContentBlockType) => {
    try {
      ...
      // displayOrder es opcional: el API calcula max(existing) + 1
