...
import { PLATFORM_SUBSCRIPTION_STATUSES, type PlatformSubscriptionStatus, PAYMENT_STATUSES } from '@workspace/shared/constants';
...
export function createPlatformSubscriptionsRepository(db: Db) {
  return {
    ...
    async findAll(filters: SubscriptionFilters = {}): Promise<PaginatedSubscriptions> {
      ...
      let countQuery = db.select({ total: count() }).from(storeSubscription);
