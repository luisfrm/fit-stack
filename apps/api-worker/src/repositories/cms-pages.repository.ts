...
export function createCmsPagesRepository(db: Db) {
  return {
    ...
    async findBySlug(organizationId: string, slug: string): Promise<ICmsPage | undefined> {
      const records = await db
        .select()
        .from(cmsPage)
        .where(and(eq(cmsPage.slug, slug), eq(cmsPage.organizationId, organizationId)));
      return records[0] as unknown as ICmsPage | undefined;
    },
...
export type CmsPagesRepository = ReturnType<typeof createCmsPagesRepository>;
