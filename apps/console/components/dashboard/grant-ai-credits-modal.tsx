 ...
 */
export function GrantAiCreditsModal({
  initialData: organization,
  onSuccess,
  trigger,
}: GrantAiCreditsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await organizationsService.grantAiCredits(
        organization.id,
        parsed,
      );
      toast.success(
        `${result.granted} créditos IA otorgados a ${organization.name}`,
      );
      ...
      toast.error(
        mutationError(
          "GrantAiCreditsModal",
          err,
          "No se pudieron otorgar los créditos IA",
        ),
      );
    } finally {
      setIsLoading(false);
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="flex items-start gap-3 rounded-xl border border-border bg-surface-2/40 px-3.5 py-3">
          ...
          <Text size="xs" variant="muted" className="leading-relaxed">
            1 crédito = 1K tokens. Los créditos se acumulan sobre el consumo del
            ciclo vigente y quedan disponibles de inmediato (el usuario refresca
            el panel).
          </Text>
        </div>
