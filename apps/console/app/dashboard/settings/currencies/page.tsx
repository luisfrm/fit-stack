...
export default function PlatformCurrenciesPage() {
  ...
  React.useEffect(() => {
    if (settings) {
      const activeCurs = settings[PLATFORM_SETTINGS_KEYS.ACTIVE_CURRENCIES];
      if (activeCurs) {
        try {
      ...
      const primaryCur = settings[PLATFORM_SETTINGS_KEYS.PRIMARY_CURRENCY];
      if (primaryCur) {
        setPrimaryCurrency(primaryCur);
      }

      const format = settings[PLATFORM_SETTINGS_KEYS.CURRENCY_FORMAT];
