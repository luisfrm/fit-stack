...
function stateBadge(state: IReceiptReportRow["state"]) {
  switch (state) {
    ...
    case "pending":
      return (
        <Badge variant="warning" size="sm" className="pointer-events-none">
          PDF pendiente
        </Badge>
      );
    case "voided":
      return (
        <Badge variant="destructive" size="sm" className="pointer-events-none">
          Anulado
        </Badge>
      );
    default:
...
export function ReceiptsReportClient({
  ...
  const [status, setStatus] = React.useState<ReceiptReportStatusFilter>(
  ...
  const currentFilters = React.useCallback(
    () => ({
      from: from || undefined,
      to: to || undefined,
      status,
  ...
  const applyFilters = () => {
    router.push(`/subscriptions/receipts?${buildParams("1").toString()}`);
  };

  const navigatePage = (newPage: number) => {
    router.push(`/subscriptions/receipts?${buildParams(String(newPage)).toString()}`);
  };

  const handleDownloadPdf = async (row: IReceiptReportRow) => {
    setDownloadingId(row.paymentId);
    try {
      const blob = await platformSubscriptionsService.downloadReceipt(row.paymentId);
      downloadBlob(blob, `${row.receiptNumber ?? row.paymentId}.pdf`);
      toast.success("Comprobante descargado");
    } catch (err) {
      toast.error(
        mutationError("ReceiptsReport", err, "No se pudo descargar el comprobante"),
      );
    } finally {
  ...
  const handleExportCsv = async () => {
    setIsExporting(true);
    try {
      const full = await platformSubscriptionsService.getReceiptsReport({
        ...currentFilters(),
        page: 1,
        limit: 1000,
      });
      ...
      downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8" }), "comprobantes-fs.csv");
  ...
  return (
    <div className="flex flex-col gap-8" data-testid="receipts-report">
      {loadError && (
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
          <Text size="sm" weight="bold" className="text-destructive">
            {loadError}
          </Text>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-border bg-card p-4">
          <Text size="xs" variant="muted" uppercase weight="bold">
            Emitidos
          </Text>
          <Text size="lg" weight="bold" className="tabular-nums">
            {initialReport.summary.issued}
          </Text>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <Text size="xs" variant="muted" uppercase weight="bold">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <Text size="lg" weight="bold" className="tabular-nums">
            {initialReport.summary.preSystem}
          </Text>
        </div>
      </div>

      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        <Text weight="bold" size="sm" uppercase>
          Totales por moneda (emitidos)
        </Text>
        ...
        {initialReport.totals.map((total) => (
          <div key={total.currency} className="flex flex-col gap-1 border-t border-border/50 pt-2">
            <div className="flex justify-between items-center">
              <Text size="sm" weight="bold" className="uppercase">
                {total.currency}
              </Text>
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-3">
          ...
          {initialReport.gaps.map((gap) => (
            ...
            >
              ...
              <Text size="sm" weight="bold" className="font-mono">
                {gap.receiptNumber}
              </Text>
              {gap.kind === "anulado" && (
                <Text size="xs" variant="muted" className="italic">
