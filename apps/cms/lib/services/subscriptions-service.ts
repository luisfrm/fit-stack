...
export const subscriptionsService = {
  ...
  getRecent: async (limit: number = 5): Promise<Array<{ name: string; createdAt: string }>> => {
    const { data } = await apiClient.get<Array<{ name: string; createdAt: string }>>('/subscriptions/recent', {
      params: { limit }
    })
    return data
  }
